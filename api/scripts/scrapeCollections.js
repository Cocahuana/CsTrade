...
// Helper to convert URL slug to proper collection name
...
// Helper to extract items from HTML by rarity
function extractItems(html) {
	...
	try {
		// The data uses JavaScript object syntax, not JSON
		// Extract item names and rarities using regex patterns
...
async function scrapeCollection(slug) {
	...
	try {
		...
		// Debug: Save first HTML to see structure
		if (slug === "the-genesis-collection") {
			const debugPath = path.join(__dirname, "debug-html.txt");
			fs.writeFileSync(debugPath, html);
			console.log(`  📄 Saved HTML to ${debugPath} for debugging`);
		}
...
// Scrape all collections with delay
async function scrapeAll() {
	...
	for (let i = 0; i < collections.length; i++) {
		...
		if (collection) {
			...
			console.log(
				`✓ ${collection.name} - ${
					Object.values(collection.items).flat().length
				} items`
			);
		}
